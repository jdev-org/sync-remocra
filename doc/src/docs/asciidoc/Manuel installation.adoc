...
*Les variables `EDP_REGISTRY_SERVER`, `EDP_REGISTRY_USERNAME` et `EDP_REGISTRY_PASSWORD` doivent être définies et exportée avant la procédure*. Elles sont nécessaires pour se connecter au registry Docker et on ne souhaite pas versionner les credentials (sécurité). Le registre est utilisé uniquement lorsque la variable `EDP_REGISTRY_SERVER` est définie.

Les images publiées suivent désormais deux règles :

* un tag immuable par build pour éviter tout écrasement (`main-<sha-court>` pour les builds de `main`, `1.43` pour une release `v1.43`) ;
* un alias flottant éventuel pour faciliter certains usages (`main` pour la branche `main`, `latest` pour la dernière release).

En exploitation, il est recommandé de configurer `EDP_VERSION` avec un tag immuable plutôt qu'avec `latest`.
...
# Exemple de version immuable de release
...
[source,sh]
----
# Exemple de version immuable
export EDP_VERSION=main-a1b2c3d4e5f6
...
Vérifier en particulier la configuration d'accès à l'API REMOcRA :

- `API_HOST` et `API_BASE_PATH`
- `API_AUTH_TYPE=keycloak`
- `API_KEYCLOAK_URL`, `API_KEYCLOAK_REALM`, `API_KEYCLOAK_CLIENT_ID` et
  `API_KEYCLOAK_CLIENT_SECRET`

Pour une installation REMOcRA V3, la même image applicative et le même fichier `common.env` doivent
être utilisés par les scripts `run-changedatacapture.sh`, `run-pushworker.sh` et par le script qui
exécute la commande applicative `notification`. Le job de notification initialise le client API
REMOcRA ; il doit donc recevoir les variables `API_*` et ne doit pas rester sur une image antérieure.
