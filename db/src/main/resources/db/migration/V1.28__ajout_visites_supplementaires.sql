...
CREATE FOREIGN TABLE edp.VUE_PEI_EDP_REMOCRA (
  ...
  type_derniere_visite CHARACTER VARYING(100),
  date_maj_p TIMESTAMP WITHOUT TIME ZONE,
  date_maj_e TIMESTAMP WITHOUT TIME ZONE,
  date_maj_qp TIMESTAMP WITHOUT TIME ZONE,
  date_maj_cf TIMESTAMP WITHOUT TIME ZONE,
  date_maj_np TIMESTAMP WITHOUT TIME ZONE
...
ADD COLUMN date_derniere_visite TIMESTAMP WITHOUT TIME ZONE,
ADD COLUMN type_derniere_visite CHARACTER VARYING(100),
ADD COLUMN date_maj_cf timestamp without time zone,
ADD COLUMN date_maj_np timestamp without time zone;
...
BEGIN
  ...
  FOR remotePei IN (SELECT gid_edp, reference, type_objet, prive, precision_pei_prive, adresse, notes_localisation, diametre, modele,
	...
	date_maj_p, date_maj_e, date_maj_qp, date_maj_cf, date_maj_np
	...
	ELSE
		...
		INSERT INTO edp.tracabilite_pei(reference, geometry1, type_objet,
		  ...
		  essai_debit, date_derniere_visite, type_derniere_visite, date_maj_p, date_maj_e, date_maj_qp, date_maj_cf, date_maj_np, date_traca) VALUES
		(remotePei.reference, NULL, remotePei.type_objet, remotePei.prive,
		 ...
		 remotePei.date_maj_e, remotePei.date_maj_qp, remotePei.date_maj_cf, remotePei.date_maj_np, current_execution) RETURNING id INTO tracaPeiId;
		...
		IF (remotePei.date_maj_qp > derniereRemontee) THEN
		  INSERT INTO edp.message(id_traca_pei, date, type) VALUES
		  (tracaPeiId, remotePei.date_maj_qp, 'DEBIT_PRESSION');
		END IF;
		IF (remotePei.date_maj_cf > derniereRemontee AND UPPER(remotePei.type_derniere_visite) = 'CONTROLE PERIODIQUE - POINT EAU INCENDIE CONTROLE REALISE') THEN
		  INSERT INTO edp.message(id_traca_pei, date, type) VALUES
		  (tracaPeiId, remotePei.date_maj_cf, 'VISITE CF');
		END IF;
		IF (remotePei.date_maj_np > derniereRemontee AND UPPER(remotePei.type_derniere_visite) != 'CONTROLE PERIODIQUE - POINT EAU INCENDIE CONTROLE REALISE') THEN
		  INSERT INTO edp.message(id_traca_pei, date, type) VALUES
		  (tracaPeiId, remotePei.date_maj_np, 'VISITE NP');
		END IF;
