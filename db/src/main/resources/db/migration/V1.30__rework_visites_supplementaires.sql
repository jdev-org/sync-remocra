...
CREATE FOREIGN TABLE edp.VUE_PEI_EDP_REMOCRA (
  ...
  date_maj_mj TIMESTAMP WITHOUT TIME ZONE
...
ADD COLUMN date_maj_mj timestamp without time zone;
...
BEGIN
  ...
  FOR remotePei IN (SELECT gid_edp, reference, type_objet, prive, precision_pei_prive, adresse, notes_localisation, diametre, modele,
	...
	ELSE
		...
		END IF;
		IF (remotePei.date_maj_qp > derniereRemontee
		    OR remotePei.date_maj_cf > derniereRemontee
		    OR remotePei.date_maj_np > derniereRemontee) THEN
		  INSERT INTO edp.message(id_traca_pei, date, type) VALUES
